import React, { useRef } from 'react';
import { Text, Box, Edges } from '@react-three/drei';
import { useFrame } from '@react-three/fiber';
import { useSimulationStore } from '../../store/simulationStore';
import { ExplosionParticles } from './ExplosionParticles';
import * as THREE from 'three';

export const StackRail: React.FC = () => {
    const stackSlots = useSimulationStore(s => s.stackSlots || []);
    const isMutating = useSimulationStore(s => s.isMutating);

    return (
        <group position={[0, 0, 0]}>
            {/* Header */}
            <Text
                position={[0, 2.6, 0]}
                fontSize={0.26}
                color="#a855f7"
                anchorX="center"
                outlineWidth={0.025}
                outlineColor="#0b0b0f"
            >
                Execution Stack
            </Text>

            {/* Single visible frame (show only latest) */}
            {stackSlots.slice(0, 1).map((slot, index) => (
                <StackFrameSlice key={slot.id} slot={slot} index={index} />
            ))}

            {/* Explosion Effect on Commit */}
            {isMutating && (
                <ExplosionParticles position={[0, 1.5, 0]} color="#a855f7" />
            )}

            {/* HandleClick floating label */}
            <Text
                position={[1.8, 0.6, 0]}
                fontSize={0.18}
                color="#cbd5e1"
                anchorX="left"
                anchorY="middle"
                outlineWidth={0.02}
                outlineColor="#0b0b0f"
            >
                handleClick()
            </Text>
        </group>
    );
};

const StackFrameSlice: React.FC<{ slot: any, index: number }> = ({ slot, index }) => {
    const meshRef = useRef<any>(null);

    useFrame((state) => {
        if (slot.isLocked && meshRef.current) {
            // High frequency glitch
            meshRef.current.position.x = (Math.random() - 0.5) * 0.05;
        }
    });

    return (
        <group position={[0, 1.4 - index * 1.5, 0]}>
            {/* Glass Slab */}
            <Box args={[3.4, 1.1, 0.18]}>
                <meshPhysicalMaterial
                    color={slot.isLocked ? "#dc2626" : "#a855f7"}
                    emissive={slot.isLocked ? "#b91c1c" : "#7c3aed"}
                    emissiveIntensity={slot.isLocked ? 0.55 : 0.28}
                    transmission={0.55}
                    thickness={0.6}
                    roughness={0.1}
                    transparent
                    opacity={slot.isLocked ? 0.75 : 0.82}
                />
                <Edges color={slot.isLocked ? "#ef4444" : "#c084fc"} />
            </Box>

            {/* Slots */}
            <group position={[-1.1, 0.1, 0.14]}>
                <Text fontSize={0.18} color="#e2e8f0" anchorX="left">count</Text>
                <Text fontSize={0.34} color="#ffffff" anchorX="left" position={[1.2, 0, 0]}>{slot.value}</Text>
            </group>
            <group position={[-1.1, -0.25, 0.14]}>
                <Text fontSize={0.16} color="#cbd5e1" anchorX="left">setCount</Text>
                <Text fontSize={0.16} color="#c084fc" anchorX="left" position={[1.2, 0, 0]}>{'()=>dispatch'}</Text>
            </group>

            {/* Locked badge */}
            {slot.isLocked && (
                <Text
                    position={[0, 0.75, 0]}
                    fontSize={0.16}
                    color="#fca5a5"
                    anchorX="center"
                    outlineWidth={0.02}
                    outlineColor="#0b0b0f"
                >
                    LOCKED SNAPSHOT
                </Text>
            )}
        </group>
    );
}
